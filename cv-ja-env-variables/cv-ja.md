<!--
この CV はフロントマターの代わりに環境変数を使用して個人情報を設定します。
md2cv は以下の環境変数を直接読み込みます：

必要な GitHub Secrets:
- CV_JA_NAME
- CV_JA_NAME_JA
- CV_JA_NAME_FURIGANA
- CV_JA_EMAIL_ADDRESS
- CV_JA_PHONE_NUMBER
- CV_JA_POST_CODE
- CV_JA_HOME_ADDRESS
- CV_JA_HOME_ADDRESS_FURIGANA
- CV_JA_EMAIL_ADDRESS2
- CV_JA_PHONE_NUMBER2
- CV_JA_POST_CODE2
- CV_JA_HOME_ADDRESS2
- CV_JA_HOME_ADDRESS2_FURIGANA
- CV_JA_DOB
- CV_JA_GENDER

ローカル開発:
md2cv はマークダウンファイルと同じディレクトリにある .env ファイルを自動的に読み込みます。
1. .env.example を .env にコピー
2. 情報を入力
3. 実行: npx md2cv -i cv-ja-env-variables/cv-ja.md -o cv-ja-env-variables/cv-ja -f cv -t both -p a4

GitHub Actions:
1. Settings > Secrets and variables > Actions に移動
2. 上記のシークレットを個人情報で追加
-->

# 概要

5年以上のフルスタック開発経験を持つソフトウェアエンジニア。スケーラブルなシステム構築とチーム育成に情熱を持つ。大規模Webアプリケーションの設計から運用まで一貫して担当し、マイクロサービスアーキテクチャへの移行プロジェクトをリード。アジャイル開発手法を活用したチームマネジメントにも強みを持ち、若手エンジニアの育成にも積極的に取り組んでいる。

# 職歴

```resume:experience
- company: 株式会社テック
  location: 東京都港区
  roles:
    - title: シニアエンジニア
      team: プラットフォーム開発部
      start: 2021-04
      end: present
      summary:
        - マイクロサービスアーキテクチャの設計・実装を担当
        - 100万ユーザー規模のシステム運用
      highlights:
        - 5名のチームリードとしてコードレビュー・メンタリング実施
        - システムパフォーマンスを40%改善
        - CI/CDパイプラインの構築によりデプロイ時間を50%短縮
- company: スタートアップABC株式会社
  location: 東京都渋谷区
  roles:
    - title: エンジニア
      team: プロダクト開発チーム
      start: 2019-04
      end: 2021-03
      summary:
        - React/TypeScriptによるフロントエンド開発
        - Node.js/PostgreSQLによるREST API実装
      highlights:
        - プロダクトチームと連携した機能開発
        - ユニットテストカバレッジを30%から80%に向上
```

# 学歴

```resume:education
- school: 東京大学大学院
  degree: 情報理工学系研究科 修士課程
  location: 東京都文京区
  start: 2019-04
  end: 2021-03
  details:
    - 分散システムとクラウドコンピューティングを専攻
    - "修士論文: コンテナオーケストレーションの最適化に関する研究"
- school: 東京大学
  degree: 工学部 情報工学科
  location: 東京都文京区
  start: 2015-04
  end: 2019-03
  details:
    - "GPA: 3.8/4.0"
    - 学部長賞受賞
    - "卒業研究: 機械学習を用いた自然言語処理の研究"
```

# スキル

```resume:skills
categories:
- category: 言語
  description: JavaScript, TypeScript, Python, Go
- category: フロントエンド
  description: React, Vue.js, HTML/CSS
- category: バックエンド
  description: Node.js, PostgreSQL, Redis
- category: インフラ
  description: AWS, Docker, Kubernetes
```

# 資格

```resume:certifications
- name: AWS ソリューションアーキテクト アソシエイト
  issuer: Amazon Web Services
  date: 2023-05
- name: 基本情報技術者
  date: 2018-10
- name: TOEIC 850点
  date: 2022-03
```

# 志望動機

貴社のクラウドネイティブ技術への取り組みと、エンジニア主導の開発文化に強く共感しております。これまで培ってきたマイクロサービス設計やチームリードの経験を活かし、貴社のプロダクト開発に貢献したいと考えております。特に、大規模システムのパフォーマンス最適化や、開発プロセスの改善において即戦力として活躍できると確信しております。

# 自己PR

- 5年以上のフルスタック開発経験と、マイクロサービスアーキテクチャの設計・実装スキル
- 5名規模のチームリードとして、コードレビューやメンタリングを通じた若手育成の実績
- システムパフォーマンス40%改善、デプロイ時間50%短縮などの具体的な成果
- AWS、Docker、Kubernetesを活用したクラウドネイティブ開発の豊富な経験

# 本人希望記入欄

勤務地は東京都内を希望いたします。リモートワークとオフィス勤務のハイブリッド形態を希望しております。
